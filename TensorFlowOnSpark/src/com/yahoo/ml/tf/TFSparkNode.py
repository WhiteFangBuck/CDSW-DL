...
def run(fn, tf_args, cluster_meta, tensorboard, queues, background):
	...
	#socket.gethostname()
        ...
        logging.info("check server to see if this task is being retried {0}".format(cluster_meta['server_addr']))
...
def train(cluster_info, qname='input'):
    ...
    def _train(iter):
        ...
        mgr = _get_manager(cluster_info, socket.gethostbyname(socket.getfqdn()) , os.getppid())
	#socket.gethostname(), os.getppid())
